...
yoda_portal_timeout               | The web server timeout for requests to the Yoda portal, in seconds (default: 60). If the intake module is enabled, the timeout value is configured using parameter `intake_extended_timeout` instead, for backwards compatibility reasons.
