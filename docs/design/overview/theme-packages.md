...
From version 1.10 onward, Yoda supports changing themes through the Admin GUI. This feature is accessible to administrators and members of the priv-admin group. To make themes available, add their packages (including templates and static files) to `/var/www/yoda/themes`, where the Admin GUI recognizes available themes by reading the folder names—as Flask scans this directory for themes. Once you select a new theme, it updates the theme configuration in `/var/www/yoda/config/admin_settings.json` under the YODA_THEME key (e.g., "YODA_THEME": "uu").
