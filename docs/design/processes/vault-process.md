...
## Copy to vault
The **retry-copy-to-vault.r** cronjob in the tools directory of the research ruleset will copy any folder with a 'PENDING' or 'RETRY' cronjob copy state to the vault and set it to 'SECURED' state when successful. The copy uses `irsync`. The metadata is copied as well. Folders with 'RETRY' status have previously failed to copy.
