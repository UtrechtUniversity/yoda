...
if os.path.exists(os.path.join(os.path.expanduser("~"),
    ...
    with open(os.path.join(os.path.expanduser("~"),
                           ".damock_server.json"), "r") as fp:
        CONFIG = json.load(fp)
else:
    with open("/etc/damock_server.json", "r") as fp:
        CONFIG = json.load(fp)
...
def dals(argv=sys.argv[1:]):
    ...
    while True:
        ...
        if line == b'':
            ...
            _dals_process_line(line, curr_dir)
    while True:
...
def _dals_process_line(line, basedir):
...
def daattr_format_attr(obj, attr=DAATTR_FIELDS, delim=" "):
    line = ''
    for f in attr:
        if line:
            line += delim
...
def daattr_long_format_attr(obj, attr=DAATTR_FIELDS, delim=None):
    fmt = "{0: >%d} : {1}" % max([len(f) for f in DAATTR_FIELDS])
    for f in attr:
        print(fmt.format(f, str(obj.get(f))))
    print()


def daattr(argv=sys.argv[1:]):
    parser = ArgumentParser(description='')
    parser.add_argument('files', type=str, nargs='*')
    ...
    parser.add_argument("-a",
                        type=str,
                        default=",".join(DAATTR_FIELDS),
                        dest="attr",
                        help=("Displays one or more of file attributes"))
    args = parser.parse_args(argv)
    if args.long:
        formatter = daattr_long_format_attr
    else:
        formatter = daattr_format_attr
    for f in args.files:
